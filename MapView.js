...
//   // Effect to insert markers when the WebView is loaded and markersList changes
//   useEffect(() => {
//     if (isLoaded && props.markersList != null && props.markersList.length > 0) {
//       props.markersList.forEach(({ID, cords, icon}) => {
//         insertMarker(mapRef, ID, cords, icon);
//       });
//     }
//   }, [isLoaded, props.markersList, mapRef]);

//   //   Effect to animate to position when the WebView is loaded and animateToPosition changes
//   useEffect(() => {
//     if (isLoaded && props.animateToPosition != null) {
//       goToPosition(mapRef, ...props.animateToPosition);
//     }
//   }, [isLoaded, props.animateToPosition, mapRef]);
...
export default function MapView(props) {
  ...
  useEffect(() => {
    if (
      ...
      props.markersList.forEach(({ID, cords, icon}) => {
        insertMarker(mapRef.current, ID, cords, icon);
      });
    }
  }, [isLoaded, props.markersList]);

  // Effect to animate to position when the WebView is loaded and animateToPosition changes
  useEffect(() => {
    if (isLoaded && props.animateToPosition != null && mapRef.current) {
      goToPosition(mapRef.current, ...props.animateToPosition);
    }
  }, [isLoaded, props.animateToPosition]);
