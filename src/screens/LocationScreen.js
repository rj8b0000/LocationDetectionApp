...
import MapLibreRN, {MapView} from '@maplibre/maplibre-react-native';
import leafletStyle from '../assets/mapstyles/leaflet-style.json';
import {Button, Text} from 'react-native-paper';

MapLibreRN.setAccessToken(null); // not required for MapLibre

const STYLE_URL = require('../assets/mapstyles/leaflet-style.json');
...
// Safe, compact point-in-polygon (expects coords as [lat, lon])
...
// Compute bounding box that includes polygon + user
const computeBounds = (polygon, user) => {
  ...
  // camera.fitBounds expects [lon, lat]
...
export default function LocationGate({navigation}) {
  ...
  // 1) Load polygon from your API
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        setLoading(true);
        const resp = await fetch('http://10.0.2.2:8080/api/coordinates');
        const data = await resp.json();
        if (!resp.ok) throw new Error('Failed to fetch polygon');
        // Your API: data[0].polygon.coordinates[0] is [[lon, lat], ...]
  ...
  // 4) GeoJSON for polygon rendering
  const polygonGeoJSON = useMemo(() => {
    if (!polygon?.length) return null;
    // MapLibre wants [lon, lat]
  ...
  return (
    <View style={{flex: 1}}>
      <MapView
        style={{flex: 1}}
        styleJSON={leafletStyle}
        compassEnabled
        logoEnabled={false}>
        <MapLibreRN.Camera ref={cameraRef} zoomLevel={12} />

        {polygonGeoJSON && (
          <MapLibreRN.ShapeSource id="poly" shape={polygonGeoJSON}>
        ...
        {/* Native user location stream + marker */}
...
//       setLoading(true);
//       setError(null);
//       const response = await fetch('http://10.0.2.2:8080/api/coordinates');
//       console.log('Response: ', response);
//       const data = await response.json();
//       if (!response.ok) {
//         throw new Error('Failed to fetch coordinates');
//       }
//       // Extract coordinates from the nested polygon.coordinates[0] and swap longitude,latitude to latitude,longitude
//       const coordinates = data[0].polygon.coordinates[0].map(coord => [
